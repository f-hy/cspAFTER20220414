...
struct noteBook {                        // the notebook
  ...
  int lineLen(int _x = cs.x) {
    return notes.at(_x - 1).size();
  }  // get the line length, default line is cs.x
...
pair<cursor, cursor> sortse(
    cursor &_sc, cursor &_ec,
    bool directlyChange = false) {  // compare two coordinate points
...
struct shiftOthers {
  ...
  shiftOthers(bool _shift = false, bool _selected = false)
      : shift(_shift), selected(_selected) {}
  void turnArea() { selectArea = sortse(recordPoint, cs); }
} so;
void printselectedArea(
    pair<cursor, cursor>
        sa) {  // test function, print the selected area's two borders
  cout << '(' << sa.first.x << ',' << sa.first.y << ") (" << sa.second.x << ','
       << sa.second.y << ")\n";
}
bool paste0(const string _s, cursor &_cs,
            bool movey = true) {  // paste within one line and no '\n'
  note.notes.at(_cs.x - 1).insert(_cs.y - 1, _s);
  if (movey) {
    _cs.y += _s.size();
  }
  if (_cs.x > note.notes.size() || _cs.y > note.notes.at(_cs.x - 1).size())
    return false;
  return true;
}
bool paste1(const deque<string> _note, cursor &_cs,
            bool move = true) {  // paste with no less than one line and a '\n'
                                 // or deal with the head and tail
  bool hasTail = (_note.back()[_note.back().size() - 1] != '\n');
  if (hasTail) {
    paste0(_note.back(), _cs, false);  // insert the tail
  }
  string tmp = note.notes.at(_cs.x - 1).substr(
      _cs.y - 1, note.lineLen(_cs.x) - _cs.y + 1);  // a copy of the line tail
...
void paste2(const deque<string> _note, cursor &_cs,
            bool move = true) {       // paste with no less than three rows
  bool hasTail = paste1(_note, _cs);  // deal with the head and tail
  for (int i = _note.size() - hasTail - 1; i > 0;
       --i) {  // deal with the middle lines
...
void remove1(pair<cursor, cursor> se) {                 // remove head and tail
  ...
  note.notes.at(se.first.x - 1) +=
      note.notes.at(se.second.x - 1);  // join the tail to head
  note.notes.erase(note.notes.begin() + se.second.x - 1);  // erase the tail
                                                           // line
}
...
void opshift() {  // operation of "SHIFT"
  ...
  if (so.shift) {  //粘滞功能关闭时
    if (so.recordPoint.x != cs.x ||
        so.recordPoint.y !=
            cs.y) {  //此时的光标位置与记录点的位置不同，进入选中状态
      so.turnArea();
      so.selected = true;
    }
    so.shift = false;
...
ll cntWord(string word, bool global = true) {  // count word in the chosed area
  ...
  if (global) {
    ...
    int lx = so.selectArea.first.x, ly = so.selectArea.first.y,
        rx = so.selectArea.second.x, ry = so.selectArea.second.y;
    if (lx + 1 < rx) {  // mid lies
      tmpi = rx - 1;
    ...
    if (note.notes.at(lx - 1).size() - ly + 1 >= wordSize) {  // head row
      ...
      if (lx == rx)
        tmpj = (ry - wordSize) < maxj ? (ry - 1) : maxj;
      else
        tmpj = maxj;  // c1
...
void opcopy() {       // operation of "COPY"
  if (so.selected) {  // selected
    ...
    if (lx == rx) {        //一行之内
      ...
      so.clipBoard.push_back(note.notes.at(lx - 1).substr(
          ly - 1, note.notes.at(lx - 1).size() - ly + 1));  // Head
      if (lx + 1 < rx) {                                    //超过2行, >=3行
        for (int i = lx; i < rx - 1; ++i) {
